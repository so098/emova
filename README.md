...
| 문제 | 원인 | 해결 | 문서 |
|------|------|------|------|
| 그리드 선택 시 DB 에러 | enum에 값이 부족 → text로 전환 | 마이그레이션 004 | |
...
| 스키마 설계 미흡 6건 | enum 조기 도입, 이중 ID, 인덱스 부재 등 | 005 마이그레이션으로 일괄 수정 | [schema-review](docs/schema-review.md) |
...
│   ├── home/             # 메인 그리드 선택
│   ├── emotion/          # 감정 캐러셀
│   ├── quest/            # 퀘스트 상태 로직 (questLogic.ts) + 훅 + UI
│   ├── question/         # 질문 목록/상세
│   ├── recommend/        # 추천 결과
│   └── reflect/          # 회고 가이드
├── store/                # Zustand 스토어 5개
├── lib/supabase/         # Supabase 클라이언트 + API 레이어 (movaFlowApi, questApi)
├── constants/            # 라우트, 질문 상수
└── utils/                # 유틸리티 함수
```
...
| [스키마 리뷰](docs/schema-review.md) | 설계 미흡점 6건과 개선 과정 |
