...
- 200줄 이하 → OK
- 200~400줄 → 역할이 하나면 허용, 여러 역할이면 분리 검토
- **400줄 초과 시 반드시 유저에게 알릴 것** — "이 파일이 N줄이고, [역할 A / 역할 B / ...]가 섞여 있어서 분리를 제안합니다" 형태로 알려줄 것
- 판단 기준은 줄 수가 아니라 **역할 수** — 한 파일이 렌더링, 상태 관리, API 호출, 모달, 메뉴 등 3개 이상 역할을 하면 분리 대상
