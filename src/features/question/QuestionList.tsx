  ...
  return (
    <div className="flex w-full max-w-(--ui-content-width) flex-col gap-4">
          ...
          {QUESTIONS.slice(0, visible).map(({ label, sub, color, Icon }, i) => (
            <motion.div
              ...
              transition={{
                ...
                stiffness: 400,
                damping: 28,
                delay: i < INITIAL_COUNT ? 0 : 0.05 * (i - visible + STEP),
              }}
              onClick={() => {
                const qs = searchParams.toString();
                const slug = encodeQuestionSlug(i);
                router.push(
                  qs ? `/question/${slug}?${qs}` : `/question/${slug}`,
                );
              }}
              whileTap={{ scale: 0.97 }}
              className="border-border-card-glass bg-surface-card-glass flex cursor-pointer items-center gap-4 rounded-2xl border px-5 py-4 shadow-[0_2px_8px_rgba(0,0,0,0.05)] backdrop-blur-lg"
            >
              <div
      ...
      {visible < QUESTIONS.length && (
        <button
          onClick={showMore}
          className="text-text-muted hover:text-brand-primary mt-1 self-center text-sm font-semibold transition-colors"
        >
          질문 더 보기
        </button>
      )}
