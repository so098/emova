...
import { User, Lock, Trophy, X, PartyPopper, MessageSquare } from "lucide-react";
...
export default function ProfileContent() {
  ...
  const reflectionCount = Array.isArray(reflectionData) ? reflectionData.length : 0;
  ...
  useEffect(() => {
    ...
    for (const a of ACHIEVEMENTS) {
      if (a.check(stats) && !unlockedKeys.has(a.key) && !pendingKeys.current.has(a.key)) {
        pendingKeys.current.add(a.key);
    ...
    <main className="flex min-h-dvh items-start justify-center px-4 pt-7 pb-8">
      <div className="flex w-full max-w-(--ui-content-width) flex-col gap-6">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-border-default bg-surface px-5 py-4">
          ...
          >
            <span className="text-sm text-text-secondary">피드백 보내기</span>
            <MessageSquare size={16} strokeWidth={2} className="text-text-muted" />
          </button>
        </div>
      </div>
          ...
          <>
            ...
            >
              ...
              <div className="w-full px-6 pb-6">
                <button
                  onClick={dismissCelebration}
                  className="w-full rounded-full bg-point py-3 text-sm font-bold text-on-point"
                >
                  확인
                </button>
              </div>
